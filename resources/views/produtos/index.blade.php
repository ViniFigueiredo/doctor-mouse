...
<form action="{{ route('produtos.search') }}" method="GET" class="mb-6 flex flex-wrap gap-2 items-center" style="margin-left: 15px; margin-right: 15px;">
    ...
    <select name="ordenar" class="p-2 border rounded focus:outline-none focus:ring-2 focus:ring-primary" style="width: 190px;">
...
<div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
    @forelse($produtos as $produto)
        <div class="bg-white border border-gray-200 rounded-xl shadow p-4 flex flex-col">
            <div class="aspect-square p-3 bg-gray-50 rounded-lg flex items-center justify-center mb-3">
                @if($produto->imagem)
            ...
            @if(!auth()->check())
                <a target="_self" href="{{ route('signin') }}" 
                   class="w-full bg-primary hover:bg-purple-800 text-white py-2 rounded text-center font-bold transition mt-auto">
